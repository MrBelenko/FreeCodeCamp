				...
				if (myTwitch.M.lastStreamsRefresh === null) {
					...
					for (var channel in self.twitchs) {
						...
						if (self.twitchs[current].existence === "existent") {
							...
							var dd = $.Deferred();
							(function(current, self) {
								$.get("https://wind-bow.gomix.me/twitch-api/streams/" + current + "?callback=?")
								.done(function(response) {
		...
		**************************************************/		
		...
		displayChannel: function(channel) {
			...
			if (channel.existence === "nonExistent") {
				...
				logoSrc = "https://res.cloudinary.com/dqzrtsqol/image/upload/v1464091895/TwitchGlitchIcon_WhiteonPurple_bt6alc.png";
			}
				...
				if (channel.logo) {
					...
					logoSrc = "https://res.cloudinary.com/dqzrtsqol/image/upload/v1464091895/TwitchGlitchIcon_PurpleonWhite_tayh1q.png";
				}
			...
			var template =
					"<a id='" + channelId + "' " + linkUrl + "class='list-group-item " + linkDisabled + statusClassStyle + " " + statusClassFilter + "' target='_blank'>" +
					"  <div class='row'>" +
